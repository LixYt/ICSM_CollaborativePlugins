...
namespace NetPlugins2Ext
{
    public partial class IcsMetroGeoViewQuery : IcsMetroUserControl
    {
        ...
        private void Load_Stations(string filter)
        {
            ...
            string FormatString = "*";
            if (yAllP.IsPresent2("POS_ID")) { FormatString += ",POS_ID,Position(NAME,LONGITUDE,LATITUDE)"; }
            if (yAllP.IsPresent2("SITE_ID")) { FormatString += ",SITE_ID,Position(NAME,LONGITUDE,LATITUDE)"; }
        ...
        private void ResizeSplitScreen()
        {
            ...
            {
                case "Horizontal":
                    SplitGeoQueryBrowser.Orientation = Orientation.Horizontal;
                    SplitGeoQueryBrowser.SplitterDistance = SplitGeoQueryBrowser.Size.Height / 2;
                    break;

                case "Vertical":
                case "":
                default:
                    SplitGeoQueryBrowser.Orientation = Orientation.Vertical;
                    SplitGeoQueryBrowser.SplitterDistance = SplitGeoQueryBrowser.Size.Width / 2;
                    break;
            }
